...
3. Test with `./sync.py --skill start`.
4. Document any adapter limitations in `config.yaml` under `notes`.
5. Update `docs/adapters.md`.

### Adding a new skill

1. Create `skills/<group>/{name}.md` with frontmatter (`skill`, `type`, `description`, `args`). Use the group that matches the skill's domain.
...
- Edit generated files (`.claude/commands/`, `.cursor/rules/`, `.agents/skills/`, `.gemini/skills/`) — edit the source skill under `skills/<group>/` instead.
