#!/usr/bin/env python3
"""
agentgate/sync.sh — generate agent-specific skill files from source skills + project config

Usage:
  ./agentgate/sync.sh                      # use .agentgate.yaml in current dir
  ./agentgate/sync.sh --config path/to/config.yaml
  ./agentgate/sync.sh --force              # overwrite customized files without prompting
  ./agentgate/sync.sh --dry-run            # preview what would be written, no changes made
  ./agentgate/sync.sh --skill start        # sync only the named skill(s), comma-separated
...
AGENTGATE_DIR = Path(__file__).parent
MARKER = "generated by agentgate/sync.sh"
...
def read_file_info(file_path):
    """
    Inspect an existing file for agentgate provenance.
    ...
    Returns (stored_hash, body_hash, is_generated) where:
      ...
      is_generated — True if the file was written by agentgate/sync.sh
...
def load_adapter(adapter_name):
    ...
    adapter_dir = AGENTGATE_DIR / 'adapters' / adapter_name
...
def sync_skill(skill_path, adapter, project_config, project_root, args):
    ...
    marker_line = f"<!-- {MARKER} | skill: {skill_name} | adapter: {adapter['name']} | hash: {h} | DO NOT EDIT — run agentgate/sync.sh to regenerate -->\n"
    ...
    if out_path.exists() and not is_generated and not args.force:
        print(f"  ⚠ {skill_name} (file exists but was not generated by agentgate — skipping, use --force to overwrite)")
        return
    ...
    if is_generated and not args.force:
        if body_hash != stored_hash:
            ...
            print(f"  ⚠ {skill_name} (customized since last sync — skipping, use --force to overwrite)")
            return
        # body_hash == stored_hash but != h: source skill changed, file unmodified → safe to regenerate

    # Write
    if args.dry_run:
        print(f"  → {skill_name} (would write to {out_path})")
    else:
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(final_content)
        action = "updated" if out_path.exists() else "written"
        print(f"  ✓ {skill_name} ({action} → {out_path})")


def main():
    parser = argparse.ArgumentParser(
        description='Generate agent-specific skill files from agentgate/skills/')
    parser.add_argument('--config', default='.agentgate.yaml',
                        help='Path to project config (default: .agentgate.yaml)')
    ...
    if not config_path.exists():
        ...
        print(f"Copy {AGENTGATE_DIR}/templates/agentgate.yaml to .agentgate.yaml and customize it.")
        sys.exit(1)
    ...
    if not adapters_list:
        print("Error: no adapters specified in config. Add 'adapters: [claude]' to .agentgate.yaml")
        sys.exit(1)

    # Determine which skills to sync
    skills_dir = AGENTGATE_DIR / 'skills'
    ...
    for adapter_name in adapters_list:
        ...
        for skill_path in skill_files:
            sync_skill(skill_path, adapter, project_config, project_root, args)
