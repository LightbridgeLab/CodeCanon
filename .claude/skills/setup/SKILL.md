...
- **Skills** (`skills/<group>/<name>/SKILL.md`) — portable workflow instructions in the Agent Skills folder format, with `main`-style tokens for project-specific values (see `config.schema.yaml`)
...
- Only modify `.codecannon.yaml` and local git config (Phase 3 signing setup). Do not touch any other file (except running `CodeCannon/sync.py`, which modifies the generated skills directories — permitted only with explicit user approval).
...
<!-- generated by CodeCannon/sync.py | skill: setup | adapter: claude | hash: 6d21a7fa | DO NOT EDIT — run CodeCannon/sync.py to regenerate -->
