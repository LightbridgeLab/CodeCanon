...
Check whether `` is currently applied to the issue. If it is **not** present, warn:

> "Issue #N does not have the  label. It may not be deployed to preview yet. Continue anyway? (yes/no)"
...
- If `` is empty (PASS case), skip `--add-label` but still run `--remove-label ""`.
- If `` is empty (FAIL case), skip `--add-label` but still run `--remove-label ""`.
