<!-- generated by agentgate/sync.sh | skill: version | adapter: claude | hash: fe7f5058 | DO NOT EDIT — run agentgate/sync.sh to regenerate -->
...
If not on `development`, abort and say: "Switch to `development` before running `/version`."
...
> "Tagged as vX.Y.Z. Run `make deploy-preview` to deploy to preview for testing. When testing is complete, run `/release`."
